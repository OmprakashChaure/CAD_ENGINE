...
def extract_dxf_data(file_path):
    # Verificar que el archivo sea estrictamente un DXF
    if not file_path.lower().endswith('.dxf'):
        print(f"Error: El archivo no es un formato DXF válido (.dxf): {file_path}")
        return None
    ...
    try:
        ...
        print(f"No se puede leer el archivo DXF: {file_path}")
        return None
    except ezdxf.DXFStructureError:
        print(f"Archivo DXF dañado o no válido: {file_path}")
        return None
    ...
    def extract_dimension_data(entity):
        ...
        if dim_type == 0:    # Dimensión lineal rotada, horizontal o vertical
            dim_data["defpoints"] = [entity.dxf.defpoint, entity.dxf.defpoint2]
        elif dim_type == 1:  # Dimensión lineal alineada
            dim_data["defpoints"] = [entity.dxf.defpoint, entity.dxf.defpoint2]
        elif dim_type == 2:  # Dimensión angular (2 líneas)
            dim_data["defpoints"] = [entity.dxf.defpoint, entity.dxf.defpoint2, entity.dxf.defpoint3, entity.dxf.defpoint4]
        elif dim_type == 3:  # Dimensión de diámetro
            dim_data["defpoints"] = [entity.dxf.defpoint, entity.dxf.defpoint2]
        elif dim_type == 4:  # Dimensión de radio
            dim_data["defpoints"] = [entity.dxf.defpoint, entity.dxf.defpoint2]
        elif dim_type == 5:  # Dimensión angular (3 puntos)
            dim_data["defpoints"] = [entity.dxf.defpoint, entity.dxf.defpoint2, entity.dxf.defpoint3]
        elif dim_type == 6:  # Dimensión angular (4 puntos)
            dim_data["defpoints"] = [entity.dxf.defpoint, entity.dxf.defpoint2, entity.dxf.defpoint3, entity.dxf.defpoint4]
        return dim_data
...
def create_dxf_file(output_path, data):
    # Forzar que la salida termine en .dxf
...
# Uso del script con nomenclatura correcta de DXF
dxf_file =r"C:\Users\Administrator\Desktop\CAD_ENGINE\CAD_ENGINE\CAD_ENGINE\data\raw_dxf\Aero_LW03_SparWeb.dxf"
output_file = r"C:\\Users\\Administrator\\Desktop\\CAD_ENGINE\\CAD_ENGINE\\CAD_ENGINE\\extracted_data.dxf"

extracted_data = extract_dxf_data(dxf_file)

# Guardar los datos extraídos en un nuevo archivo DXF
if extracted_data:
    create_dxf_file(output_file, extracted_data)
