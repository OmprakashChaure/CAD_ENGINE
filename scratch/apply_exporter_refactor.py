...
# Find the end of _mask_context_leakage
# We look for the end_anchor from the start_idx onwards
# Since python read_text normalizes newlines to \n, let's check for standard \n in string
end_anchor_norm = "                new_obj[k] = self._mask_context_leakage(v, target_val, target_prop, original_property)\n            return new_obj\n        elif isinstance(obj, list):\n            return [self._mask_context_leakage(x, target_val, target_prop, original_property) for x in obj]\n        else:\n            return obj"

end_idx = content.find(end_anchor_norm, start_idx)
if end_idx == -1:
    ...
    end_idx += len(end_anchor_norm)

# Define the replacement code block
replacement_code = r"""    def _build_tasks_from_semantic(
        self,
        semantic_record: Any,
        result: Dict[str, Any],
    ) -> List[Dict]:
        ...
        # Helper: Clean dictionaries/lists recursively of empty/null-only structures (Implementation Goal 3 & 4)
        ...
        def add_semantic_property_task(
            ...
            # ── TASK-SPECIFIC CONTEXT SELECTION (Implementation Goal 2) ──
            filtered_context = {}
            filtered_context["part_type"] = semantic_record.part_type
            filtered_context["feature_type"] = feature.feature_class if feature else "span_dimension"
            
            # Sibling parameters of the feature itself
            if feature:
                filtered_context["feature_parameters_visible"] = {
                    k: v for k, v in feature.parameters.items()
                    if k != property_name and k not in {"positions", "center"}
                }
                
            # Filter neighbor dimensions to strip neighbor_id keys (Goal 3)
            clean_neighbor_dims = []
            ...
            if target_prop == "spacing":
                # Spacing Tasks: Prioritize spacing, patterns, centers, neighbors, topology
                filtered_context["topology_neighbors"] = rich_ctx.get("topology_neighbors", [])
                filtered_context["neighbor_dimensions"] = clean_neighbor_dims
                filtered_context["overall_dimensions"] = overall_dims_copy
                
                symmetries = []
                if hasattr(semantic_record, "relationships") and semantic_record.relationships:
                    for rel in semantic_record.relationships:
                        if rel.relationship_type == "mirror_symmetry":
                            params = rel.parameters or {}
                            symmetries.append({
                                "role": "mirrored_feature",
                                "axis": params.get("axis"),
                                "axis_position": params.get("axis_position"),
                                "pair_count": len(rel.feature_ids) // 2 if rel.feature_ids else 0
                            })
                filtered_context["symmetries"] = symmetries
                
                if feature_candidate_id:
                    filtered_context["relationships"] = self._get_engineering_relationships_for_candidate(feature_candidate_id, result)
                if context_extra:
                    filtered_context.update(context_extra)

            elif target_prop == "thread_size":
                # Thread Size Tasks: Prioritize concentric details, neighbor feature dimensions, bore info
                filtered_context["neighbor_dimensions"] = clean_neighbor_dims
                ...
                if hasattr(semantic_record, "relationships") and semantic_record.relationships:
                    for rel in semantic_record.relationships:
                        if rel.relationship_type == "concentric":
                            feature_ids = rel.feature_ids or []
                            diameters = []
                            for fid in feature_ids:
                                feat = next((f for f in features if f.feature_id == fid), None)
                                if feat:
                                    fparams = feat.parameters or {}
                                    d = (fparams.get("bore_diameter") or fparams.get("boss_diameter") or 
                                         fparams.get("outer_diameter") or fparams.get("diameter") or 
                                         fparams.get("inner_diameter") or fparams.get("o_ring_diameter") or
                                         fparams.get("port_diameter"))
                                    if d:
                                        diameters.append(d)
                            if diameters:
                                concentric_features.append({
                                    "role": "concentric_element",
                                    "inner_diameter": min(diameters),
                                    "outer_diameter": max(diameters)
                                })
                filtered_context["concentric_features"] = concentric_features
                
                if feature_candidate_id:
                    eng_rels = self._get_engineering_relationships_for_candidate(feature_candidate_id, result)
                    filtered_context["relationships"] = [r for r in eng_rels if r.get("pattern_type") == "concentric"]
                
                filtered_context["concentric_context"] = rich_ctx.get("concentric_context")

            elif target_prop == "wall_thickness":
                # Wall Thickness Tasks: Prioritize inner/outer profile sizes, adjacent spans, nested contours
                filtered_context["neighbor_dimensions"] = clean_neighbor_dims
                filtered_context["overall_dimensions"] = overall_dims_copy
                filtered_context["hierarchy"] = rich_ctx.get("hierarchy")
                
                nesting_context = None
                if feature and hasattr(semantic_record, "hierarchy") and semantic_record.hierarchy:
                    nodes = semantic_record.hierarchy.get("nodes", [])
                    fid = feature.feature_id
                    node = next((n for n in nodes if n.get("candidate_id") == fid), None)
                    if node:
                        nesting_context = {
                            "role": "contained_feature" if node.get("parent_id") else "outer_contour",
                            "nesting_depth": node.get("nesting_depth", 0),
                            "parent_role": "outer_contour" if node.get("nesting_depth", 0) == 1 else "inner_contour" if node.get("parent_id") else None,
                            "sibling_count": len(node.get("children_ids", []))
                        }
                filtered_context["nesting_context"] = nesting_context
            ...
            elif target_prop in ("profile_dimension", "pocket_dimension", "slot_dimension"):
                # Profile, Slot, Pocket Tasks: Prioritize contour hierarchy, bounding geometry, topology
                filtered_context["topology_neighbors"] = rich_ctx.get("topology_neighbors", [])
                filtered_context["neighbor_dimensions"] = clean_neighbor_dims
                filtered_context["overall_dimensions"] = overall_dims_copy
                filtered_context["hierarchy"] = rich_ctx.get("hierarchy")
                ...
                # Default / Diameter / Count tasks: Maintain rich standard profile parameters
                filtered_context["neighbor_dimensions"] = clean_neighbor_dims
                filtered_context["overall_dimensions"] = overall_dims_copy
            ...
            # Strip graph IDs to expose only physical engineering context
            sanitized_context = self._sanitize_graph_identifiers(filtered_context)
            
            # Mask leakage (Implementation Goal 6)
            masked_context = self._mask_context_leakage(sanitized_context, value, target_prop, original_property=property_name)
            
            # Recursively strip empty dictionaries, lists, and null fields (Goal 3 & 4)
            cleaned_context = _clean_empty_and_nulls(masked_context)
            if cleaned_context is None:
                cleaned_context = {}
                
            # QUALITY VALIDATION: Reject tasks without any physical reasoning context
            has_reasoning = any(k not in ("part_type", "feature_type") for k in cleaned_context.keys())
            if not has_reasoning:
                logger.warning(
                    f"DatasetExporter: Rejected task {task_type} for drawing {drawing_id} "
                    f"due to insufficient reasoning context after noise removal."
                )
                return
                ...
                if cg_center and len(cg_center) >= 2 and math.dist(cand_center[:2], cg_center[:2]) < 1.0:
                    ...
                    if cand_r is not None:
                        ...
                        engineering_rels.append({
                            ...
                            "radius_sequence": [round(r, 4) for r in sorted(cg_radii)]
                        })
            ...
            if candidate_id in cids:
                ...
                if circular_pattern:
                    engineering_rels.append({
                        ...
                        "radius": round(circular_pattern.get("pattern_radius"), 4) if circular_pattern.get("pattern_radius") is not None else None,
    ...
    def _map_to_v21_family(self, task_type: str) -> Optional[str]:
        ...
        if prop in ("nominal_thread", "nominal_diameter", "thread_pitch", "thread_size", "port_thread") or "thread" in prop or prop.startswith("m") or "thd" in prop:
    ...
    def _mask_context_leakage(
        ...
        if isinstance(obj, dict):
            ...
            for k, v in obj.items():
                ...
                new_obj[k] = self._mask_context_leakage(v, target_val, target_prop, original_property)
            return new_obj
        elif isinstance(obj, list):
            return [self._mask_context_leakage(x, target_val, target_prop, original_property) for x in obj]
        else:
...
print("Successfully refactored dataset_pipeline.py!")
